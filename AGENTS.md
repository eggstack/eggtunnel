...
- Version lines: workspace manifest is `0.2.0` (M012 candidate); published crates.io line / tag / GitHub release remain `0.1.0` until owner-authorized publication (see `docs/DISTRIBUTION.md`). Dependency snippets in `docs/API.md` + `docs/EMBEDDING.md` intentionally stay at `version = "0.1"` (the resolvable published line) until then — do not bump them early. Wire protocol stays v1.0 independent of crate version.
