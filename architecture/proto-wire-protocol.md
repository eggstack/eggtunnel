...
| No session state machine | `crates/eggtunnel/src/client.rs:1037-1088` (handshake) + `crates/eggtunnel/src/client.rs:1157-1237` (control loop), `crates/eggtunnel/src/server.rs:1152-1214` (control loop) + first-frame gates `crates/eggtunnel/src/server.rs:723-725`, `crates/eggtunnel/src/server.rs:809-811`, `crates/eggtunnel/src/server.rs:946-963` enforce ordering with `UnexpectedMessage` | `decode_frame` never returns `UnexpectedMessage` (`crates/eggtunnel-proto/src/lib.rs:474-525`); the variant is only constructed by client/server (`crates/eggtunnel/src/client.rs:1042`, `crates/eggtunnel/src/client.rs:1205`, `crates/eggtunnel/src/client.rs:1237`, `crates/eggtunnel/src/server.rs:725`, `crates/eggtunnel/src/server.rs:811`, `crates/eggtunnel/src/server.rs:933`, `crates/eggtunnel/src/server.rs:961`, `crates/eggtunnel/src/server.rs:1214`) |
...
|---:|---|---|---|---|
| 1 | `ClientHello` (`crates/eggtunnel-proto/src/lib.rs:232`, `crates/eggtunnel-proto/src/lib.rs:271-275`) | `version: ProtocolVersion`, `capabilities: Capabilities` | client → server | Handshake opener. Client sends first (`crates/eggtunnel/src/client.rs:1028-1036`); server requires it as the first frame on a control stream (`crates/eggtunnel/src/server.rs:723-725` QUIC, `crates/eggtunnel/src/server.rs:943-963` TCP/TLS dispatch). |
| 2 | `ServerHello` (`crates/eggtunnel-proto/src/lib.rs:233`, `crates/eggtunnel-proto/src/lib.rs:276-280`) | `version`, `capabilities` (same shape as `ClientHello`) | server → client | Handshake answer (`crates/eggtunnel/src/server.rs:1063-1070` sends; `crates/eggtunnel/src/client.rs:1037-1044` validates major version). |
| 3 | `Auth` (`crates/eggtunnel-proto/src/lib.rs:234`, `crates/eggtunnel-proto/src/lib.rs:281-285`) | `token: Vec<u8>` (private, `bounded_bytes`, ≤4096 B; redacted `Debug`) | client → server | Credential presentation inside the already-established secure transport (`docs/PROTOCOL.md:36-37`; sent at `crates/eggtunnel/src/client.rs:1046-1052`). |
| 4 | `AuthOk` (`crates/eggtunnel-proto/src/lib.rs:235`, `crates/eggtunnel-proto/src/lib.rs:317-320`) | `session_id: SessionId` | server → client | Authentication success + session binding (`crates/eggtunnel/src/server.rs:1133` sends; `crates/eggtunnel/src/client.rs:1053-1055` extracts `session_id`, else `TunnelError::Authentication`). |
| 5 | `RegisterService` (`crates/eggtunnel-proto/src/lib.rs:236`, `crates/eggtunnel-proto/src/lib.rs:321-327`) | `service_id: ServiceId`, `name: ServiceName`, `requested_bind: RequestedBind`, `target: TcpTarget` | client → server | Registration request, one per service (initial loop at `crates/eggtunnel/src/client.rs:1064-1089`, dynamic at `crates/eggtunnel/src/client.rs:1260-1269`; server validates, binds, spawns `run_service` at `crates/eggtunnel/src/server.rs:1154-1189`). Note: `target` is client-owned metadata; server never dials it (comment at `crates/eggtunnel/src/server.rs:1169`). |
| 6 | `RegisterAck` (`crates/eggtunnel-proto/src/lib.rs:237`, `crates/eggtunnel-proto/src/lib.rs:328-332`) | `service_id`, `effective_bind: EffectiveBind { address: [u8;16], port: u16 }` | server → client | Registration success with server-chosen bind (`crates/eggtunnel/src/server.rs:1189` sends; initial client path requires `ack.service_id == service.id` at `crates/eggtunnel/src/client.rs:1077-1084`, dynamic path correlates via `take_ack` at `crates/eggtunnel/src/client.rs:1202-1221`). |
| 7 | `UnregisterService` (`crates/eggtunnel-proto/src/lib.rs:238`, `crates/eggtunnel-proto/src/lib.rs:333-336`) | `service_id` | client → server | Deregistration (`crates/eggtunnel/src/client.rs:1284-1292` sends on `ClientCommand::Unregister`; `crates/eggtunnel/src/server.rs:1191-1200` cancels service, removes pending). Unknown IDs are silently tolerated server-side (remove-if-present). |
| 8 | `Open` (`crates/eggtunnel-proto/src/lib.rs:239`, `crates/eggtunnel-proto/src/lib.rs:337-341`) | `service_id`, `connection_id: ConnectionId` | server → client (control plane) | Per-external-connection demand: server listener accepted, server queues pending and sends `Open` (`crates/eggtunnel/src/server.rs:1348-1352` via `open_rx`, forwarded at `crates/eggtunnel/src/server.rs:1218-1221`); client receives at `crates/eggtunnel/src/client.rs:1159-1174`, resolves `service_id`, else `OpenReject(code 1)` at `crates/eggtunnel/src/client.rs:1164`; semaphore-full → `OpenReject(code 2)` at `crates/eggtunnel/src/client.rs:1172`. |
| 9 | `OpenReject` (`crates/eggtunnel-proto/src/lib.rs:240`, `crates/eggtunnel-proto/src/lib.rs:342-346`) | `connection_id`, `code: u16` | client → server | Negative answer to `Open` (control-loop rejects at `crates/eggtunnel/src/client.rs:1164`, `crates/eggtunnel/src/client.rs:1172`; data-path failure at `crates/eggtunnel/src/client/open.rs:83-86`; `crates/eggtunnel/src/server.rs:1202-1207` drops the pending entry). Codes are untyped `u16` (1 = unknown service, 2 = resource-exhausted in current client). |
| 10 | `Ping` (`crates/eggtunnel-proto/src/lib.rs:241`, `crates/eggtunnel-proto/src/lib.rs:347-350`) | `nonce: u64` | either direction (keepalive) | Client heartbeat ticker (`crates/eggtunnel/src/client.rs:1114-1140`; 20 s default at `crates/eggtunnel/src/common.rs:299`); each side answers `Ping` with `Pong{nonce}` (`crates/eggtunnel/src/client.rs:1195`, `crates/eggtunnel/src/server.rs:1209-1211`). |
| 11 | `Pong` (`crates/eggtunnel-proto/src/lib.rs:242`, `crates/eggtunnel-proto/src/lib.rs:351-354`) | `nonce: u64` | either direction (answer) | Echoes the `Ping` nonce; client matches it against the outstanding ping and ignores non-matching `Pong`s (`crates/eggtunnel/src/client.rs:1196-1201`). |
| 12 | `Drain` (`crates/eggtunnel-proto/src/lib.rs:243`, `crates/eggtunnel-proto/src/lib.rs:355-358`) | `deadline_ms: u32` | either direction (graceful shutdown) | Client sends on cancellation (`crates/eggtunnel/src/client.rs:1123-1127`); server sends on shutdown paths (`crates/eggtunnel/src/server.rs:598`, `crates/eggtunnel/src/server.rs:695`); receipt breaks the control loop on both sides (`crates/eggtunnel/src/client.rs:1232-1236`, `crates/eggtunnel/src/server.rs:1213`, forwarded `Drain` breaks at `crates/eggtunnel/src/server.rs:1218-1221`). |
| 13 | `Error` / `ErrorMessage` (`crates/eggtunnel-proto/src/lib.rs:244`, `crates/eggtunnel-proto/src/lib.rs:359-363`) | `code: u16`, `diagnostic: BoundedDiagnostic` (≤256 B) | server → client | Terminal/negative ack: auth failure (`crates/eggtunnel/src/server.rs:1087-1091`, code 4) and registration failures via `write_registration_error` (`crates/eggtunnel/src/server.rs:1301-1308`, call sites `crates/eggtunnel/src/server.rs:1160` code 5, `crates/eggtunnel/src/server.rs:1166` code 1, `crates/eggtunnel/src/server.rs:1172` code 2, `crates/eggtunnel/src/server.rs:1176` code 3); client maps handshake-time `Error` to `TunnelError::Authorization` (`crates/eggtunnel/src/client.rs:1086-1087`) and dynamic `Error` with a pending registration to `registration_error()` (`crates/eggtunnel/src/client.rs:1222-1229`, mapping at `crates/eggtunnel/src/client.rs:1345-1351`), or to `TunnelError::Authorization` when nothing is pending (`crates/eggtunnel/src/client.rs:1229`); other unexpected messages hit `UnexpectedMessage` (`crates/eggtunnel/src/client.rs:1237`). Note the enum variant is `Message::Error` but the struct is `ErrorMessage` (`crates/eggtunnel-proto/src/lib.rs:385`, `crates/eggtunnel-proto/src/lib.rs:521`). |
| 14 | `DataHello` (`crates/eggtunnel-proto/src/lib.rs:245`, `crates/eggtunnel-proto/src/lib.rs:364-369`) | `session_id: SessionId`, `service_id: ServiceId`, `connection_id: ConnectionId` | client → server (data plane) | First frame on each **data** connection, then opaque relay bytes (`docs/PROTOCOL.md:36-39`; client sends at `crates/eggtunnel/src/client/open.rs:66`; server requires it first on data streams at `crates/eggtunnel/src/server.rs:809-811` QUIC / `crates/eggtunnel/src/server.rs:946-958` TCP dispatch, correlated jointly at `crates/eggtunnel/src/server.rs:966-1031`). Wrong-session / unknown-connection / service-or-expiry mismatches are rejected and counted (`crates/eggtunnel/src/server.rs:978-1022`), covered by `crates/eggtunnel/src/server_tests/tcp.rs:1248`, `crates/eggtunnel/src/server_tests/tcp.rs:1309`, `crates/eggtunnel/src/server_tests/quic.rs:287`, `crates/eggtunnel/src/server_tests/quic.rs:406`, `crates/eggtunnel/src/server_tests/quic.rs:530`. |
...
| `ServiceId` (`crates/eggtunnel-proto/src/lib.rs:58-59`) | `pub struct ServiceId(pub u64)`, `Copy`, `Debug`, `Eq`/`Hash` | no generation in proto; client-chosen per service (initial `crates/eggtunnel/src/client.rs:1065-1069`, dynamic `crates/eggtunnel/src/client.rs:1260-1264` copy `service.id`); server treats duplicate IDs/names as errors (`crates/eggtunnel/src/server.rs:1163-1166`) | transparent `u64` — non-secret multiplexing key |
...
| Crate version | workspace `0.2.0` line (`crates/eggtunnel-proto/Cargo.toml:4` inherits `version.workspace`; root `Cargo.toml:6` sets `version = "0.2.0"`) | `docs/PROTOCOL.md:3-6` states the split: crate line `0.2.0` candidate, published crates.io line `0.1.0`, wire `1.0` |
...
- [ ] **Wire vs. crate version confusion.** Wire `1.0` ≠ crate `0.2.0`
  (root `Cargo.toml:6`; published line `0.1.0`, see `docs/PROTOCOL.md:3-6`). Do not gate wire behavior on
...
- [ ] **Untyped codes.** `OpenReject.code: u16` (`crates/eggtunnel-proto/src/lib.rs:345`) and
  ...
  assign meaning ad hoc (e.g. codes 1/2 at `crates/eggtunnel/src/client.rs:1164`,
  `crates/eggtunnel/src/client.rs:1172` plus `crates/eggtunnel/src/client/open.rs:83-86`; codes 5/1/2/3 at `crates/eggtunnel/src/server.rs:1160`,
  `crates/eggtunnel/src/server.rs:1166`, `crates/eggtunnel/src/server.rs:1172`,
  `crates/eggtunnel/src/server.rs:1176`). Document new codes centrally or risk silent
...
- [ ] **`SessionId` copy-paste across planes.** The same `SessionId` appears in `AuthOk`
  ...
  or replay confusion (joint verification at `crates/eggtunnel/src/server.rs:966-1031`). Server-side tests already cover wrong-session/replay/stale `DataHello`
...
- [ ] **`ServiceId` collisions.** Proto does not allocate or deduplicate `ServiceId`; server
  rejects duplicates per session (`crates/eggtunnel/src/server.rs:1163-1166`). Client-side ID
...
- [ ] **`UnexpectedMessage` is a protocol-state signal, not a decode error.**
  ...
  machines (`crates/eggtunnel/src/client.rs:1042`, `crates/eggtunnel/src/client.rs:1205`,
  `crates/eggtunnel/src/client.rs:1237`, `crates/eggtunnel/src/server.rs:725`,
  `crates/eggtunnel/src/server.rs:811`, `crates/eggtunnel/src/server.rs:933`,
  `crates/eggtunnel/src/server.rs:961`, `crates/eggtunnel/src/server.rs:1214`). Fuzzing `decode_frame` alone
