...
|---|---|---|---|
| Secret credential | `SecretToken` | always (+ `expose()` gated, see §7) | `crates/eggtunnel/src/common.rs:17` |
| Local service mapping (private side) | `ClientService` | always | `crates/eggtunnel/src/common.rs:50` |
| Server-side policy entry | `ServiceSpec` | always | `crates/eggtunnel/src/common.rs:75` |
| Listener admission policy | `BindPolicy` | struct always; enforcement server-only | `crates/eggtunnel/src/common.rs:83` |
| Observability | `Snapshot`, `Counters`, `ResourceLimits`, `TerminationCategory` | `Snapshot`/`ResourceLimits`/`TerminationCategory` always; `Counters` gated | `crates/eggtunnel/src/common.rs:134`, `crates/eggtunnel/src/common.rs:200` |
| Error vocabulary | `TunnelError` + `termination_category()` | always | `crates/eggtunnel/src/common.rs:273`, `crates/eggtunnel/src/common.rs:301` |
| Server-only enforcement | `verify_token`, `bind_to_socket`, `permits_address`, `permits_port` | `server` only | `crates/eggtunnel/src/common.rs:318`, `crates/eggtunnel/src/common.rs:324`, `crates/eggtunnel/src/common.rs:348` |

Design intent, corroborated by `docs/SECURITY.md:9-17` and `docs/ARCHITECTURE.md:1-6`:

- The wire DTOs live one layer down in `eggtunnel-proto` (runtime-neutral, bounded). `common.rs` lifts them into runtime-facing config/policy/observability types: it imports `EffectiveBind, RequestedBind, ServiceId, ServiceName, SessionId, TcpTarget` at `crates/eggtunnel/src/common.rs:10`.
...
Defined at `crates/eggtunnel/src/common.rs:17`.

### 2.1 Validation — `crates/eggtunnel/src/common.rs:20-28`
...
### 2.2 Redacted `Debug` — `crates/eggtunnel/src/common.rs:36-40`
...
- `Clone, Eq, PartialEq` are derived (`crates/eggtunnel/src/common.rs:16`) but `Debug` is manual. There is deliberately no `Display`, no `Serialize`, and no accessor returning an owned copy.
- This matches the documented guarantee in `docs/SECURITY.md:6-7`: "secret-bearing configuration Debug output is redacted."

### 2.3 Zeroize-on-drop — `crates/eggtunnel/src/common.rs:42-46`
...
### 2.4 `expose()` visibility — `crates/eggtunnel/src/common.rs:30-33`
...
`ClientService` — `crates/eggtunnel/src/common.rs:50-55`, constructor `crates/eggtunnel/src/common.rs:57-71`:
...
`ServiceSpec` — `crates/eggtunnel/src/common.rs:75-79`, constructor `crates/eggtunnel/src/common.rs:124-132`:
...
Typed admission policy for server-owned service listeners. Struct at `crates/eggtunnel/src/common.rs:83-91`.

### 4.1 Every field

| Field | Type | Meaning |
|---|---|---|
| `allow_public_addresses` | `bool` | Master switch for non-loopback bind addresses. `false` (default) means loopback only. Set from `ServerConfig.allow_public_service_binds` in `Server::bind` (`crates/eggtunnel/src/server.rs:110-114`) and the `bind_websocket` / `bind_quic` variants. |
| `allowed_addresses` | `Vec<[u8; 16]>` | Exact IPv6-byte allowlist. **Empty permits any address allowed by `allow_public_addresses`** (`crates/eggtunnel/src/common.rs:85`). Non-empty is an intersection: the address must both pass the loopback/public gate and be a member of the list (`crates/eggtunnel/src/common.rs:350-353`). |
| `allowed_port_ranges` | `Vec<(u16, u16)>` | Inclusive port ranges. **Empty permits every nonzero port** (`crates/eggtunnel/src/common.rs:87`). Port 0 (ephemeral) is never governed by ranges — see `allow_ephemeral_ports`. |
| `allow_ephemeral_ports` | `bool` | Whether `port == 0` (OS-assigned ephemeral) is permitted. Default `true`. Checked first in `permits_port` (`crates/eggtunnel/src/common.rs:356-365`). |
| `max_services_per_session` | `usize` | Per-session service ceiling enforced at registration (`crates/eggtunnel/src/server.rs:985`). Default 64. Upper-bounded by `ResourceLimits` (see `validate()`). |

### 4.2 Defaults — `crates/eggtunnel/src/common.rs:112-122`
...
`BindPolicy::loopback_only()` at `crates/eggtunnel/src/common.rs:94-96` is `Self::default()`. This is the `docs/SECURITY.md:9-10` default: "Service binds are loopback only unless `allow_public_service_binds` is explicitly enabled."

### 4.3 `validate()` rules — `crates/eggtunnel/src/common.rs:98-109`

Called by `Server::bind_with_policy` (`crates/eggtunnel/src/server.rs:125`) and the websocket variant (`crates/eggtunnel/src/server.rs:148`) before any TLS/listener setup. Fails with `TunnelError::Configuration("bind policy is invalid")` if **any** of:

1. `max_services_per_session == 0`, or
2. `max_services_per_session > ResourceLimits::default().services_per_session` (i.e. > 64), or
...
`permits_address` — `crates/eggtunnel/src/common.rs:350-353` (`#[cfg(feature = "server")]`):
...
`permits_port` — `crates/eggtunnel/src/common.rs:356-365` (`#[cfg(feature = "server")]`):
...
`bind_to_socket` — `crates/eggtunnel/src/common.rs:324-346` (`#[cfg(feature = "server")]`, `pub(crate)`):

- `RequestedBind::Loopback { port }` → checks `permits_port` only, then binds `SocketAddr::V6(SocketAddrV6::new(Ipv6Addr::LOCALHOST, port, 0, 0))` (`crates/eggtunnel/src/common.rs:331-336`). Loopback never consults `allow_public_addresses`/`allowed_addresses` — it is always address-permitted.
- `RequestedBind::Ip { address, port }` → converts with `Ipv6Addr::from(*address)`, checks `permits_address(*address, ip.is_loopback()) && permits_port(*port)`, then binds `SocketAddrV6::new(ip, port, 0, 0)` (`crates/eggtunnel/src/common.rs:337-343`). An `Ip` request carrying `::1` is treated as loopback even though it arrived via the `Ip` variant.
- Either rejection returns `TunnelError::Authorization` (which the registration loop translates into `rejected += 1` plus `write_registration_error(code 2)` at `crates/eggtunnel/src/server.rs:999`).
...
Call site: exactly one, the `RegisterService` handler at `crates/eggtunnel/src/server.rs:997`. Policy is therefore evaluated per registration, before `TcpListener::bind` at `crates/eggtunnel/src/server.rs:1001`.
...
| any | `max_services_per_session = 0` or `> 64`, or range with `start == 0` / `start > end` | ❌ `Configuration` at bind time | `validate()` rejects the whole policy before serving |
| any | `max_services_per_session = 4`, 5th `RegisterService` | ❌ registration error + `rejected += 1`, `ResourceExhausted` termination | `services.len() >= bind_policy.max_services_per_session` at `crates/eggtunnel/src/server.rs:985-990` (covered by test with `max_services_per_session: 4` at `crates/eggtunnel/src/server.rs:3814`) |

Authentication success never grants bind permission by itself (`docs/SECURITY.md:17`): auth is checked at `crates/eggtunnel/src/server.rs:917`, policy at `crates/eggtunnel/src/server.rs:997`, on every registration, against the session's policy.

---

## 5. `Snapshot` / `Counters` / `ResourceLimits`

### 5.1 `ResourceLimits` — `crates/eggtunnel/src/common.rs:176-198`

Immutable hard ceilings for the current runtime profile. `Clone, Copy, Debug, Eq, PartialEq`.

| Field | Default | Mirrored constant | Enforced at |
|---|---|---|---|
| `sessions` | 128 | `MAX_SESSIONS = 128` (`crates/eggtunnel/src/server.rs:39`) | Session admission (`crates/eggtunnel/src/server.rs:951`) |
| `services_per_session` | 64 | `BindPolicy::default().max_services_per_session = 64` (`crates/eggtunnel/src/common.rs:119`); `MAX_SERVICES = 64` client-side (`crates/eggtunnel/src/client.rs:38`) | Registration cap (`crates/eggtunnel/src/server.rs:985`); client config validation (`crates/eggtunnel/src/client.rs:476`); `BindPolicy::validate` upper bound (`crates/eggtunnel/src/common.rs:100`) |
| `pending_per_session` | 128 | `MAX_PENDING_PER_SESSION = 128` (`crates/eggtunnel/src/server.rs:40`) | `pending.len() >= MAX_PENDING_PER_SESSION` (`crates/eggtunnel/src/server.rs:1163`) |
| `active_connections_per_session` | 128 | `MAX_ACTIVE_CONNECTIONS_PER_SESSION = 128` (`crates/eggtunnel/src/server.rs:41`) | `connection_admission` semaphore (`crates/eggtunnel/src/server.rs:944`); stream admission (`crates/eggtunnel/src/server.rs:493`, `crates/eggtunnel/src/server.rs:605`) |
| `accepted_handshakes` | 64 | `MAX_HANDSHAKES = 64` (`crates/eggtunnel/src/server.rs:43`) | Handshake semaphore + guard (`crates/eggtunnel/src/server.rs:408`, `crates/eggtunnel/src/server.rs:509`, `crates/eggtunnel/src/server.rs:711-724`) |
| `client_open_tasks` | 128 | `MAX_OPEN_TASKS = 128` (`crates/eggtunnel/src/client.rs:39`) | Client `JoinSet` semaphore (`crates/eggtunnel/src/client.rs:946`) |
| `control_queue` | 128 | `CONTROL_QUEUE = 128` both sides (`crates/eggtunnel/src/server.rs:42`, `crates/eggtunnel/src/client.rs:40`) | `mpsc::channel(CONTROL_QUEUE)` server (`crates/eggtunnel/src/server.rs:970`) and client (`crates/eggtunnel/src/client.rs:947`) |

`Snapshot.resource_limits` is always `ResourceLimits::default()` (`crates/eggtunnel/src/common.rs:249`) — limits are not per-instance configurable; the snapshot echoes the compiled-in profile. The server test at `crates/eggtunnel/src/server.rs:2917` asserts `server_snapshot.resource_limits.sessions == MAX_SESSIONS`, pinning this coherence.

### 5.2 `Snapshot` — `crates/eggtunnel/src/common.rs:134-157`
...
| `resource_limits` | `ResourceLimits` | Always the default profile (see §5.1). |
...
### 5.3 `Counters` — `crates/eggtunnel/src/common.rs:200-223`

`pub(crate)`, `Clone, Default`, gated on `any(feature = "client", feature = "server")`. All counters are `Arc<atomic>` so `Clone` shares state across tasks; `last_termination` and `binds` are `Arc<Mutex<…>>` (low contention: written on termination/b bind events, cloned wholesale on snapshot).

Field correspondence (`Counters` → `Snapshot`): `connected→connected`, `sessions→active_sessions`, `services→registered_services`, `pending→pending_connections`, `active_connections→active_connections`, `open_tasks→active_client_open_tasks`, `handshakes→active_handshakes`, six `high_water_*` (note the names compress: `high_water_pending`, `high_water_open_tasks`), `task_panics`, `last_termination`, `reconnects→reconnects`, `rejected→rejected_connections`, `bytes_*`, `binds→effective_binds`.

### 5.4 `snapshot()` semantics — `crates/eggtunnel/src/common.rs:227-256`
...
- `effective_binds` is cloned under lock; cost is O(services). `resource_limits` is stamped as `ResourceLimits::default()`.

### 5.5 `record_termination` / `record_join_result` — `crates/eggtunnel/src/common.rs:258-271`
...
### 6.1 `TunnelError` variants — `crates/eggtunnel/src/common.rs:273-299`

`#[derive(Debug, Error)]` via `thiserror`. Display strings are fixed, secret-free literals except for the wrapped `#[from]` sources (`io::Error`, `ProtocolError`).

| Variant | Display | Typical origin |
|---|---|---|
| `Configuration(&'static str)` | `invalid configuration: {0}` | Bad token bytes, bad `BindPolicy`, missing runtime, oversized PEM (`crates/eggtunnel/src/server.rs:348-349` checks PEM against `MAX_FRAME_BYTES`), bad client service count |
...
| `Authorization` | `service was rejected by server policy` | Unit; covers both policy rejection (`bind_to_socket` failure) and resource-capacity rejection (`MAX_SESSIONS` overflow at `crates/eggtunnel/src/server.rs:953` returns `Authorization` after recording `ResourceExhausted` — see mapping note below) |
...
| `ResourceExhausted` | `runtime resource limit was reached` | Unit; caps, full control queue, saturated semaphores |
| `PeerClosed` | `peer closed the connection` | Unit; clean remote close during relay |

### 6.2 `termination_category()` mapping — `crates/eggtunnel/src/common.rs:301-316`

`TerminationCategory` itself is defined at `crates/eggtunnel/src/common.rs:159-172` (`Clean, Cancelled, Timeout, Authentication, Authorization, Protocol, Transport, Target, ResourceExhausted, PeerClosed, Internal`).
...
| `Authorization` | `Authorization` | 1:1 — **including** the `MAX_SESSIONS` overflow path, which records `ResourceExhausted` via `record_termination` but returns `Authorization` as the `TunnelError` (`crates/eggtunnel/src/server.rs:951-953`). The snapshot's `last_termination` and the returned error therefore disagree by design on that path; assert on the right one. |
...
### 6.3 `verify_token` constant-time semantics — `crates/eggtunnel/src/common.rs:318-322`
...
`crates/eggtunnel/Cargo.toml:16-23`: `default = ["client", "tls"]`; `client`, `server`, `tls`, `quic` (implies client+server), `websocket` (implies client+server), `outbound-proxy` (implies client), `mtls`.
...
| `Snapshot`, `TerminationCategory`, `ResourceLimits`, `TunnelError`, `termination_category()` | none | Error/observability vocabulary always linked |
| `use std::net::SocketAddr` (`crates/eggtunnel/src/common.rs:2-3`) | `server` | Client-only builds do not monomorphize socket mapping |
| `SecretToken::expose` (`crates/eggtunnel/src/common.rs:30`) | `any(client, server)` | With neither feature, the secret cannot leave the type at all (no accessor exists) |
| `Counters` struct + `snapshot`/`record_termination`/`record_join_result` (`crates/eggtunnel/src/common.rs:200-271`), atomic imports (`crates/eggtunnel/src/common.rs:4-8`) | `any(client, server)` | `--no-default-features` build has `Snapshot` as a pure data type with no producer |
| `verify_token` (`crates/eggtunnel/src/common.rs:318`), `bind_to_socket` (`crates/eggtunnel/src/common.rs:324`), `permits_address` (`crates/eggtunnel/src/common.rs:350`), `permits_port` (`crates/eggtunnel/src/common.rs:356`) | `server` | Client-only builds cannot evaluate policy or compare tokens; `BindPolicy::validate` (un-gated) remains callable for fail-fast config checks |
| `lib.rs` module wiring (`crates/eggtunnel/src/lib.rs:7-14`) | `client` / `server` per module | `common` is always compiled; `wire_io` only with either side; `Client*` re-exports need `client`, `Server*` need `server` |
...
- [ ] Every `RequestedBind → TcpListener::bind` path goes through `bind_to_socket` (`crates/eggtunnel/src/common.rs:324`). Today there is exactly one call site (`crates/eggtunnel/src/server.rs:997`); QUIC/WSS variants reuse the same registration loop — confirm any new `bind_*` variant does too.
- [ ] `allow_public_addresses` defaults to `false`; any change to `ServerConfig.allow_public_service_binds` plumbing (all four `bind*` constructors at `crates/eggtunnel/src/server.rs:109-166`) preserves opt-in publicity.
- [ ] `BindPolicy::validate` is called before serving in every constructor. A new constructor that skips it could admit `max_services_per_session = 0` (denial of all registrations) or `> 64` (ceiling above `ResourceLimits`), or port ranges containing 0.
...
- [ ] `Ip{ address: ::1 }` is intentionally treated as loopback (`ip.is_loopback()` at `crates/eggtunnel/src/common.rs:339`). Do not "fix" this into a rejection without also handling IPv4-mapped `::ffff:127.0.0.1`, which `is_loopback()` does *not* flag — changing either behavior alters the loopback allowlist surface.
