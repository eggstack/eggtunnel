...
Remaining narrow gaps after transport qualification:

- WebSocket is still not qualified as TCP half-close equivalent because
  WebSocket's protocol-level close closes the entire connection. Qualification
...
No other correctness or security gaps remain open against the
...
and `eggtunnel` are published at `0.2.0` (see
